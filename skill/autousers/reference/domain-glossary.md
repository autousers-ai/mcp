...
**Rating** — One persona's structured score on one comparison, including its per-dimension results. The atomic unit of cost: one rating ≈ $0.105–$0.158.
...
**Per-rating cost** — ~$0.105 (URL stimulus, SSE) to ~$0.158 (URL stimulus, SxS). One rating = one autouser × one comparison. Different stimulus types (image, file) have different rates.
