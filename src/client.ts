...
export class AutousersApiError extends Error {
  constructor(
    ...
    /**
     * The envelope's stable `error.code` — `INSUFFICIENT_QUOTA`,
     * `runs_already_queued`, and so on. Carried because `type` is too coarse
     * to act on: several unrelated refusals share `invalid_request_error`, and
     * a tool that wants to handle one of them specifically (rather than fail
     * the whole call) has no other way to tell them apart.
     */
...
// ---------------------------------------------------------------------------
//
// `api()` is JSON-only (`Accept: application/json`, JSON body decode). The
// chat-driven draft endpoints (`/api/v1/{autousers,templates}/draft-from-prompt`)
// stream `text/event-stream` instead, so they need a separate helper that:
//
//   1. Resolves the bearer the same way `api()` does (per-request context →
//      env), so the same auth surface works under stdio + HTTP.
//   2. Returns the raw `Response` so the caller can drive the SSE parser
//      itself — chunked events have a non-trivial protocol (event/data
//      framing, chunk re-assembly across reads) that we want callers to
//      own without us guessing their framing rules.
//   3. Pre-validates `res.ok`; on non-2xx it parses the JSON error body
//      (Next.js `apiError()` envelopes everything pre-stream errors as
//      JSON) into the same `AutousersApiError` we raise from `api()`,
//      so tool handlers can use one `try/catch` regardless of transport.
//
// The HTTP method is always POST (every draft-from-prompt route is POST).
...
export async function apiStream(
  ...
  if (!res.ok) {
    ...
    try {
      ...
      // non-JSON error body — surface the status line below
    }
    const message =
