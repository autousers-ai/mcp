 ...
 * estimate from the measured per-rating token mix documented in
 * `docs/strategy/PRICING.md` §3 and surface it in tool responses so the caller
 * (and the LLM driving it) sees the spend before it happens.
 *
 * Numbers below are anchored on `gemini-3.7-flash` for both navigation and
 * dimensional judging — the same combination the autouser runner uses in
 * production. Source of truth lives in
 * `lib/gemini-pricing.ts` in the main app, with the measured token mix in
 * `docs/strategy/PRICING.md` §3. Keep this file in step when either changes.
 */
...
/**
 * Per-rating cost basis (USD), applying today's model rates to the measured
 * token mix from `docs/strategy/PRICING.md` §3. URL is the most expensive
 * because Computer Use
 * navigation runs against `gemini-3.7-flash`. IMAGE skips navigation
...
export const COST_ESTIMATE_BASIS = {
  models: {
    navigation: "gemini-3.7-flash",
    judging: "gemini-3.7-flash",
  ...
  perRatingUsdByStimulus: {
    // Navigation leg: ~24.4k input + 10.6k output on gemini-3.7-flash's
    // introductory $0.75/$3.75 rates (through 2026-12-31) = $0.05805.
    // Judging leg re-measured after two changes landed together — the
    // recording now reaches the judge as a gs:// URI (more input tokens) and
    // judging moved to gemini-3.7-flash (cheaper per token). Net measured
    // judging cost on a real 17-turn session with a 331s recording: $0.0202.
    // Rounded up to $0.022 for headroom. Add compliance $0.001 + GKE $0.003
    // + logs $0.001.
    URL: 0.085,
    IMAGE: 0.02, // judging $0.015 + compliance $0.001 + GKE $0.003 + logs $0.001 (no navigation)
    VIDEO: 0.028, // judging $0.015 + ~30s video tokens $0.008 + compliance $0.001 + GKE $0.003 + logs $0.001
    GIF: 0.025, // judging $0.015 + ~10s GIF tokens $0.005 + compliance $0.001 + GKE $0.003 + logs $0.001
   ...
   * `totalTokensApprox` rollup. URL ~= 35k navigation + 31k judging (the
   * judge now receives the session recording, not five screenshots).
   */
   ...
   */
  // Through 2026-12-31, if all ~24.4k navigation input tokens hit the
  // $0.075/M context-cache rate, input falls by ~$0.0165; amortising 30% of
  // GKE saves another ~$0.0009. $0.090 leaves a buffer for cache storage and
  // token variance.
  postOptimisationTargetUsdSseUrl: 0.07,
} as const;
...
export function estimateRunCost(input: EstimateInput): CostEstimate {
  ...
  return {
    ...
    note:
      ...
      "Basis: measured navigation token mix at current model rates plus loaded judging and infrastructure costs.",
