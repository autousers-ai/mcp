...
const autouserRunArtifactsGetShape = {
  evaluationId: z.string().min(1).describe("Evaluation id (URL path)."),
  runId: z.string().min(1).describe("Autouser run id (URL path)."),
};
...
export function registerAutousers(server: McpServer): void {
  ...
  // -------------------------------------------------------------------
  // autouser_run_artifacts_get
  // -------------------------------------------------------------------
  server.registerTool(
    "autouser_run_artifacts_get",
    {
      title: "Get artifacts for a single autouser run",
      description:
        "Fetch the artifacts produced by one autouser run: the model's structured `ratings` (per-dimension scores, selected factors, justification), the session `transcript`, the screen-recording `video`, and the per-turn `screenshots`. SSE evals return flat `transcript`/`video`/`screenshots`; SxS evals also return a `comparisons[]` array with Side A / Side B media per comparison. Media URLs are short-lived signed GCS links (≈1h TTL) — fetch/display them promptly and re-call this tool to refresh expired ones. Runs that haven't produced media yet (or static-stimulus image/gif runs, which capture no video) return empty media arrays with `artifactsStatus:'ok'`; `ratings` are still present. Example: { evaluationId, runId } (use a runId from autouser_status_get).",
      inputSchema: autouserRunArtifactsGetShape,
      outputSchema: genericObjectShape,
      annotations: {
        readOnlyHint: true,
        destructiveHint: false,
    ...
    async ({ evaluationId, runId }) => {
      try {
        const { data, requestId } = await api(
          `/api/v1/evaluations/${encodeURIComponent(evaluationId)}/autouser-runs/${encodeURIComponent(runId)}/artifacts`
        );
        return ok(data, requestId);
      } catch (err) {
        return fail(err);
