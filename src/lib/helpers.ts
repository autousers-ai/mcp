...
export interface EvalRow {
  ...
  links?: EvalLinks;
  /**
   * Caller-supplied custom dimension id → the id the server actually
   * persisted. Present on the `POST /evaluations` response only.
   *
   * Custom dimensions become real `Dimension` rows whose primary key is a
   * server-issued cuid, so a caller's `clues-helpful` cannot survive as the
   * id — every downstream join (ratings, results, agreement) keys on the
   * cuid. Rendering this map is what lets an assistant tell the user, in the
   * same turn, which of their ids became which, instead of the rewrite being
   * discovered later when an external analysis fails to join.
   */
  dimensionIdMap?: Record<string, string>;
}

/**
 * Render the caller-id → persisted-id table, or `null` when there is nothing
 * worth saying (no custom dimensions, or every id survived unchanged).
 */
export function renderDimensionIdMap(
  map: Record<string, string> | undefined
): string | null {
  if (!map) return null;
  const remapped = Object.entries(map).filter(([from, to]) => from !== to);
  if (remapped.length === 0) return null;
  const rows = remapped
    .map(([from, to]) => `| \`${from}\` | \`${to}\` |`)
    .join("\n");
  return [
    "**Custom dimension ids were rewritten** — join external data on the persisted id:",
    "",
    "| Your id | Persisted id |",
    "| --- | --- |",
    rows,
  ].join("\n");
}
...
export function renderEvalSummary(ev: EvalRow): string {
  ...
  const idMapTable = renderDimensionIdMap(ev.dimensionIdMap);
  if (idMapTable) lines.push("", idMapTable);
  return lines.join("\n");
}
