...
export function registerEvaluationResources(server: McpServer): void {
  ...
  server.registerResource(
    ...
    }),
    {
      ...
      description:
        "Inter-rater agreement statistics for an evaluation. `insufficient` marks the cases where no figure exists at all (no ratings, one rater, no shared items); `kappa_available` says whether a chance-corrected statistic could be computed on top of the agreement percentage, and `overall_kappa_is_mean` whether that statistic is a weighted mean of several kappas rather than one.",
      mimeType: "application/json",
    },
    async (uri, variables) => {
      try {
